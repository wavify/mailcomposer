    ...
    for(var i=0, len=addresses.length; i<len; i++){
        ...
        if(address.address){
            ...
            if(!address.name){
                ...
                values.push('"' + address.name.replace(/(["\\])/g, '\\$1')
                    +'" <'+address.address+'>');
            }
        }
    }
...
MailComposer.prototype._generateBodyStructure = function(){
    ...
    if(this._message.useMixed){
        ...
        node.headers.push(["Content-Type", "multipart/mixed; charset=utf-8; boundary=\""+node.boundary+"\""]);
    ...
    if(this._message.useAlternative){
        ...
        node.headers.push(["Content-Type", "multipart/alternative; charset=utf-8; boundary=\""+node.boundary+"\""]);
        if(currentNode){
            currentNode.childNodes.push(node);
            node.parentNode = currentNode;
        }else{
    ...
    if(this._message.useRelated){
        ...
        node.headers.push(["Content-Type", "multipart/related; charset=utf-8; boundary=\""+node.boundary+"\""]);
        if(currentNode){
            currentNode.childNodes.push(node);
            node.parentNode = currentNode;
        }else{
...
MailComposer.prototype._createAttachmentComponent = function(attachment){
    ...
    node.encoded = attachment.encoded;
    ...
    node.headers.push(["Content-Type", contentType.join("; ")]);
    if (!attachment.isIgnoreContentDisposition) {
...
MailComposer.prototype._flattenMimeTree = function(){
    ...
    function walkTree(node, level){
        ...
        if(node.encoded) {
            contentObject.encoded = node.encoded;
        }
    ...
    if(element.filePath){
        if(element.filePath.match(/^https?:\/\//)){
            this._serveStream(urlFetch(element.filePath, {userAgent: element.userAgent}),
                element.encoded ? "utf-8" : element.contentEncoding, callback);
        }else if(element.encoded){
            this._serveEncodedFile(element.filePath, element.contentEncoding, callback);
        }else{
            this._serveFile(element.filePath, element.contentEncoding, callback);
        }
        return;
    }else if(element.streamSource){
        this._serveStream(element.streamSource, element.encoded ? "utf-8" : element.contentEncoding,
            callback);
        return;
    }
...
/**
 * <p>Pipes an encoded file to the e-mail stream</p>
 *
 * @param {String} filePath Path to the file
 * @param {Function} callback Callback function to run after completion
 */
MailComposer.prototype._serveEncodedFile = function(filePath, encoding, callback){
    fs.stat(filePath, (function(err, stat){
        ...
        this._serveStream(stream, "utf-8", callback);
